...
int main(void) {
    ...
    if (unitary_value != 0 && (product_quantity >= 15 || total_value >= 40)) {
        ...
        printf("Você recebeu 15/100 de desconto, o valor total do produto será de R$%.2f,00\n", total_value);
    } else {
        printf("Você não esta apto a receber desconto, o valor total ficará em R$%.2f,00", total_value);
    }
}